...
// import { FirebaseService } from 'src/app/services/_helpers/firebase/firebase.service';
...
export class PostJobComponent implements OnInit, AfterViewInit {
  ...
  btnEnabledForVehical: boolean = false;
  categoryBtnEnabled: boolean = false;
  ...
  constructor(
    ...
    this.ngZone.run(() => {
      ...
      this.loc.onPopState(() => {
        let postFormValues = this.postJobForm.controls;
        if (this.step == 'step2') {
          history.pushState(null, null, window.location.href);
          this.step = 'step1';
          ...
          if (this.selectedDate) {
            ...
            let month = this.selectedDate.toLocaleString('default', {
              month: 'short',
            });
  ...
  async getOrderDetail(order_id) {
    let order = (await this.storage.getKey('order')) as any;
    if (order) {
  ...
  navigateToPaymentScreen() {
    ...
    if (this.postJobForm.valid) {
      let newJobData = this.postJobForm.value;
  ...
  async callForAmount() {
    ...
    await this.consumerService
      ...
      .then((res: any) => {
        let total = parseFloat(res.total.toFixed(2));
    ...
    if (files) {
      ...
      for (let file of files) {
        ...
        reader.onload = async (e: any) => {
          ...
          let link = await this.imageCompressService.compressImage(
            compressImages
          );
